...
const EditDraftComponent = ({ model, allClassData, announceId }) => {
  ...
  useEffect(() => {
    async function getAnnouce() {
      const data = await getAnnounceAction(announceId);
  ...
  const handleUpdate = async (data) => {
    ...
    const updateInfo = {
      ...
      postDate: date != null ? date : new Date(announce?.postDate),
      linkAttachments:
    ...
    try {
      const newUpadate = await updateAnnounceAction(announceId, updateInfo);
  ...
  return (
    <Modal isOpen={true} isDismissable={false} size="3xl">
      <ModalContent>
        {(onClose) => (
          <>
            <form onSubmit={handleSubmit(handleUpdate)}>
              <ModalHeader className="flex flex-col gap-1 text-[24px] text-[#387ADF]">
                <div className="text-2xl font-bold flex justify-between items-center mt-2">
                  <div className="text-primary"> Edit Draft Announcement</div>
                  <div variant="light">
                    <Image
                      onClick={model}
                      src={cancel}
